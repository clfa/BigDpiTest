(function() {
    ...
    var isProduction = !/^ppe\.|review\.|localhost/.test(location.hostname);
    var techProfileDomain = "techprofile.microsoft.com";
    var techProfileUrl = isProduction ? techProfileDomain : "ppe." + techProfileDomain;
    var isNewSignIn = location.hostname.indexOf("techprofile") !== -1 && getMeta("page_type") !== "Register" || getMeta("new-sign-in") === "true";
    ...
    function tryLoadUser() {
        ...
        return fetch(url, {
            ...
            if (isNewSignIn && response.status === 404) {
                redirectToTechProfileRegisterUrl();
            }
            return response;
        }).then(function(response) {
    ...
    function redirectToTechProfileRegisterUrl() {
        window.location.href = "https://" + techProfileUrl + "/" + msDocs.data.userLocale + "/register";
    }
    ...
    function renderBranchSelector() {
        ...
        if (msDocs.data.context.branches) {
            ...
            var dropdown = createBranchSelectorHtml(branchList);
            dropdown.change(function(e) {
                var targetBranchName = event.target.value;
                if (targetBranchName) {
                    cookies.set("CONTENT_BRANCH", targetBranchName);
                    window.location.search = "?branch=" + encodeURIComponent(targetBranchName);
                }
            });
            dropdown.val(cookies.get("CONTENT_BRANCH"));
            var branchSelector = $("\n\t\t\t<li>\n\t\t\t</li>");
            branchSelector.append(dropdown);
    ...
    function login$1(navigate) {
        ...
        return doOAuthFlow(args).then(tokenApi.tryLoad);
    }
    function azureOAuthLogout() {
        var args = {
            type: "azure-logout",
            ...
            var processTokens = function(tokens) {
                if (tokens === null) {
                    ...
                    if (token) {
                        ...
                        throw new Error('Cloud Shell /accesstokens response did not contain the azure sandbox tenant "' + azureSandbox.value.tenantId + '".');
                    }
    ...
    function renderLogin(container, processTokens) {
        ...
        loginButton.onclick = function() {
            ...
            login$1(false).then(function(tokens) {
                if (tokens) {
                    reportAuthorized();
                }
    ...
    var CloudShell = function() {
        function CloudShell(isPowerShell) {
            ...
            azureOAuthLogin(this.element).then(function() {
                ...
                _this.element.innerHTML = '\n\t\t\t\t\t<div class="cloud-shell-header is-' + (isPowerShell ? "powershell" : "bash") + ' level is-mobile has-margin-bottom-none">\n\t\t\t\t\t\t<div class="level-left">\n\t\t\t\t\t\t\t<div class="level-item">\n\t\t\t\t\t\t\t\t<button title="' + escape(loc.restart) + '" class="button is-black is-radiusless" data-bi-name="restart">\n\t\t\t\t\t\t\t\t\t<span class="is-size-7 docon docon-navigate-refresh" aria-role="presentation"></span>\n\t\t\t\t\t\t\t\t\t<span class="is-size-7 visually-hidden">' + escape(loc.restart) + '</span>\n\t\t\t\t\t\t\t\t</button>\n\t\t\t\t\t\t\t\t<h2 class="is-size-7 has-text-weight-normal has-padding-left-small cloud-shell-header-title">Azure Cloud Shell</h2>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<div class="level-right">\n\t\t\t\t\t\t\t<div class="level-item">\n\t\t\t\t\t\t\t\t<a class="button is-black is-radiusless" data-bi-name="feedback" href="https://aka.ms/cloudshellfeedback">\n\t\t\t\t\t\t\t\t\t<span class="is-size-7">' + escape(loc.feedback) + '</span>\n\t\t\t\t\t\t\t\t\t<span class="icon docon docon-navigate-external" aria-role="presentation"></span>\n\t\t\t\t\t\t\t\t</a>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t</div>\n\t\t\t\t\t<iframe\tclass="cloud-shell-frame"\n\t\t\t\t\t\t\t\t\tsrc="' + cliPageOrigin + "?trustedAuthority=" + location$1.origin + "&embed=true&feature.azureconsole.ostype=" + (isPowerShell ? "windows" : "linux") + "&l=" + lang + '"\n\t\t\t\t\t\t\t\t\tframeborder="0">\n\t\t\t\t\t</iframe>';
    ...
    var server = "https://docslearning-browse-ppe.azurewebsites.net";
    function getBrowseContent(criteria) {
        var query = {
            environment: "ppe",
            locale: criteria.locale,
            terms: criteria.terms,
            facet: criteria.facets || [ "Products", "Roles", "ResourceType", "Levels" ],
            $filter: createODataFilterOptions(criteria.filter),
            $orderBy: criteria.order
        };
        var url = server + "/search?" + toQueryString(query, true);
        var init = {
            method: "GET"
        };
    ...
    var facetListTemplate = function(facets, filterCriteria) {
        ...
        return keys.map(function(key) {
            return '\n\t\t<div>\n\t\t<button class="c-glyph closed icon-var-" aria-expanded="true" aria-controls="">' + key + "</button>\n\t\t" + facetTemplate(key, facets[key], filterCriteria) + "\n\t\t</div>";
        }).join("");
    };
    var facetTemplate = function(facetName, facet, filterCriteria) {
        var list = filterCriteria[facetName];
        return "\n\t\t<ul>\n\t\t\t" + facet.map(function(f) {
            return '\n\t\t\t\t<li>\n\t\t\t\t<label class="checkbox">\n\t\t\t\t\t<input ' + (list && list.indexOf(f.value) > -1 ? 'checked="checked"' : "") + ' name="' + facetName + '" ' + (f.children && f.children.length ? "" : 'value="' + f.value + '"') + ' type="checkbox"> ' + f.value + " (" + f.count + ")\n\t\t\t\t</label>\n\t\t\t\t" + (f.children && f.children.length ? facetTemplate(facetName, f.children, filterCriteria) : "") + "\n\t\t\t</li>\n\t\t\t";
        }) + "\n\t\t</ul>\n\t";
    };
    var listTemplate = function(items) {
        return items.length ? "<ul>" + items.map(listItemTemplate).join("") + "</ul>" : "<p>No results found.</p>";
    };
    var listItemTemplate = function(item) {
        return '\n\t<li>\n\t\x3c!-- img src="' + item.iconUrl + '" / --\x3e\n\t<p><a href="' + item.url + '">' + item.title + "</a> [" + item.resourceType + "] (" + item.durationInMinutes + " min)</p>\n\t" + item.description + "\n\t<p>" + item.numberOfChildren + " children</p>\n\t</li>";
    };
    var filterCriteria = {};
    function contentBrowserPage() {
        var startingTerm = parseQueryString().term;
        eventBus.subscribe(SearchRequestEvent, handleSearchRequestEvent);
        window.addEventListener("popstate", function() {
            eventBus.publish(new SearchRequestEvent({
                terms: parseQueryString().term
            }));
        });
        eventBus.publish(new SearchRequestEvent({
            terms: startingTerm
        ...
        contentLoaded.then(function() {
            ...
            facetList.addEventListener("click", function(event) {
                if (event.target instanceof HTMLInputElement && event.target.type === "checkbox") {
                    ...
                    filterCriteria[event.target.name] = target_1.checked ? list.concat(target_1.value) : list.filter(function(x) {
                        return x != target_1.value;
                    });
                }
            });
    ...
    function handleSearchRequestEvent(event) {
        getBrowseContent(event.criteria).then(handleSearchResponseEvent);
    }
    function handleSearchResponseEvent(result) {
        contentLoaded.then(function() {
            document$1.getElementById("refine-content").innerHTML = facetListTemplate(result.facets, filterCriteria);
            document$1.getElementById("results").innerHTML = listTemplate(result.results);
        });
    }
    var SearchRequestEvent = function() {
        function SearchRequestEvent(_a) {
            var _b = _a.terms, terms = _b === void 0 ? "" : _b, _c = _a.filter, filter = _c === void 0 ? {} : _c, _d = _a.order, order = _d === void 0 ? "LastModified desc" : _d;
            this.criteria = {
                locale: msDocs.data.userLocale,
                terms: terms,
                filter: filter,
    ...
    var Quiz = function() {
        function Quiz(form, bus) {
            ...
            this.questions = form.querySelectorAll(".possible-answer");
            ...
            Array.from(this.questions).forEach(function(question) {
                question.addEventListener("click", function(event) {
                    resetChoiceStatus(question.parentElement);
                    question.classList.add("is-selected");
                });
            });
            function resetChoiceStatus(choiceBlock) {
                for (var i = 0; i < choiceBlock.children.length; i++) {
                    var question = choiceBlock.children[i];
                    question.classList.remove("is-selected");
                    question.classList.remove("is-incorrect");
            ...
            bus.subscribe(UnitValidatedEvent, this.handleValidationEvent.bind(this));
        ...
        Quiz.prototype.handleValidationEvent = function(event) {
            ...
            var questionFeedbackElement = this.form.querySelector(".question-feedback");
            if (!event.passed) {
                questionFeedbackElement.innerHTML = "Try again!";
            } else {
                var questions = document.querySelectorAll(".possible-answer.is-selected");
                ...
                for (var i = 0; i < questions.length; i++) {
                    questions[i].classList.add("is-correct");
                }
    ...
    var UnitValidatedEvent = function() {
        function UnitValidatedEvent(response) {
            this.updated = response.updated;
            this.passed = response.passed;
            this.details = response.details;
        }
        return UnitValidatedEvent;
    }();
    var prodServerUrl = "https://docs.microsoft.com/api/learn";
    var developServerUrl = "https://ppe.docs.microsoft.com/api/learn";
    ...
    function getUserProgressFromUid(uid) {
        var query = {
            branch: branch,
            uids: uid
        };
        var url = getServerUrl() + "/progress?" + toQueryString(query);
        var init = {
            method: "GET",
            credentials: "include"
        };
    ...
    function putUnitProgress(unitId, details) {
        ...
        var url = getServerUrl() + "/units/" + unitId + "/progress?" + toQueryString(query);
    ...
    function populateBreadcrumb(identifier, module) {
        if (msDocs.data.context.chromeless) {
            return;
        }
        var pageKind = getMeta("page_kind");
        var breadHolder = document.querySelector(".breadcrumbs");
        Array.from(breadHolder.querySelectorAll("[remove-on-navigate]")).forEach(function(li) {
            return li.remove();
        });
        function template(title, link) {
            return '\n\t\t\t<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" remove-on-navigate>\n\t\t\t\t' + (link ? '<a href="/' + link + '" itemprop="name">' + title + "</a>" : '<span itemprop="name">' + title + "</span>") + "\n\t\t\t</li>\n\t\t";
        }
        var breadCrumb = template(identifier);
        if (pageKind === "unit") {
            breadCrumb = template(module.title, msDocs.data.userLocale + module.url) + breadCrumb;
        }
        if (module && module.parents.length === 1) {
            var parentBreadcrumb = template(module.parents[0].title, msDocs.data.userLocale + module.parents[0].url);
            breadCrumb = parentBreadcrumb + breadCrumb;
        }
        breadcrumbsPromise.then(function() {
            breadHolder.insertAdjacentHTML("beforeend", breadCrumb);
            var mobileBreadcrumb = breadHolder.querySelector(".mobile-breadcrumb");
            if (mobileBreadcrumb) {
                breadHolder.querySelector(".mobile-breadcrumb").className = "";
            }
            breadHolder.querySelector("li:nth-last-child(2)").classList.add("mobile-breadcrumb");
        });
    }
    var moduleLoaded;
    ...
    var CombinedProgressService = function() {
        function CombinedProgressService(user, bus) {
            this.user = user;
            this.bus = bus;
        ...
        CombinedProgressService.prototype.validateQuiz = function(unitUid, answerData) {
            ...
            return authStatusDetermined.then(function() {
                ...
                _this.bus.publish(new UnitValidatedEvent(response));
                if (response.passed) {
                    _this.bus.publish(new ProgressUpdatedWithPutEvent(unitUid));
                }
                return response;
        ...
        CombinedProgressService.prototype.validateTask = function(unitUid, taskValidations) {
            ...
            return authStatusDetermined.then(function() {
                ...
                if (response.passed) {
                    _this.bus.publish(new ProgressUpdatedWithPutEvent(unitUid));
                    validateMessage.innerText = loc.youNailedIt;
                } else {
                    validateMessage.innerText = loc.taskHasErrors;
                }
                return response;
            });
        };
    ...
    var RemoteValidationService = function() {
        ...
        RemoteValidationService.prototype.getProgressByUid = function(uid) {
            return getUserProgressFromUid(uid);
        };
    ...
    var LocalProgressService = function() {
        ...
        LocalProgressService.prototype.getProgress = function() {
            var _this = this;
            return this.moduleUidLoaded.then(function(moduleUid) {
                var data = _this.getFromStorage();
                if (data && data.moduleUid === moduleUid) {
        ...
        LocalProgressService.prototype.validateQuiz = function(unitUid, answerData) {
            ...
            return putUnitProgress(unitUid, answerData).then(function(quizResponse) {
                var data = _this.getFromStorage();
                return _this.moduleUidLoaded.then(function(moduleUid) {
                    var unitInfo = {
                        unitUid: unitUid,
                        detail: answerData
                    };
                    ...
                    return convertToQuizUpdatedResponse(unitUid, answerData, quizResponse.passed);
                });
            });
        };
    ...
    function convertToQuizUpdatedResponse(unitUid, gradedQuestions, passed) {
        return {
            updated: true,
            passed: passed,
            achievements: [],
    ...
    function handleXpTag(progressBadge, learnItem, bus) {
        var xp = progressBadge.querySelector(".xp-tag-xp");
        xp.textContent = learnItem.points + " " + loc.xp;
        if (learnItem.points) {
            progressBadge.classList.remove("is-hidden");
        }
        if (learnItem.status === "completed") {
    ...
    function updateXpTag(unitId, progressBadge, progress) {
        var lookup = createProgressLookup(progress);
        if (lookup[unitId]) {
            progressBadge.classList.add("is-complete");
        }
    }
    function updateXpTagWithPut(unitId, progressBadge) {
        if (unitId === getMeta("uid")) {
            progressBadge.classList.add("is-complete");
        }
    }
    ...
    function learningPathPage() {
        ...
        var bus = new EventBus();
        var pathXpBadge = document.querySelector(".xp-tag.path-tag");
        var combinedProgressService = new CombinedProgressService(user, bus);
        Promise.all([ loadLearningPathInformation(pathId), contentLoaded ]).then(function(_a) {
            ...
            handleXpTag(pathXpBadge, learningPath, bus);
            ...
            bus.subscribe(ProgressUpdatedEvent, function(event) {
                updateXpTag(learningPath.uid, pathXpBadge, event.standardProgress);
            });
            combinedProgressService.getProgressByUid(learningPath.uid).then(function(data) {
            ...
            user.whenAuthenticated().then(function() {
                displayRemaingingModules(learningPath.modules);
    ...
    function populateModuleCard(module) {
        var template = function(units) {
            return '\n\t\t<ul class="has-margin-left-none is-unstyled">\n\t\t\t' + module.units.map(function(unit) {
                return '\n\t\t\t\t<li class="barLink is-block is-short is-narrow has-margin-bottom-medium">\n\t\t\t\t\t<a class="unit-title is-size-5 is-size-6-mobile is-flex flex-align-start" href="/' + msDocs.data.userLocale + unit.url + '">' + unit.title + '</a>\n\t\t\t\t\t<span class="unit-duration is-size-8 has-margin-top-small is-flex flex-align-end has-text-extra-subtle">' + unit.durationInMinutes + "min</span>\n\t\t\t\t</li>\n\t\t\t";
            }).join("") + "\n\t\t</ul>\n\t";
        };
    ...
    function displayRemaingingModules(modules) {
    ...
    function modulePage() {
        ...
        Promise.all([ loadModuleInformation(moduleId), contentLoaded ]).then(function(_a) {
            ...
            displayModuleDuration(module.durationInMinutes);
    ...
    function displayRelatedLearningPaths(parents) {
        var template = function(paths) {
            return '\n\t<h2 class="title is-size-6 is-marginless-bottom">' + loc.relatedLearningPaths + '</h2>\n\t<ul class="is-marginless-top is-marginless-left is-marginless-right">\n\t\t' + paths.map(function(path) {
    ...
    function promptAzureSandbox(container, promptActivate) {
        if (promptActivate === void 0) {
            promptActivate = true;
        }
        if (azureSandbox.value) {
            return Promise.resolve(azureSandbox.value);
        }
        renderSpinner(container, "Loading Azure Sandbox...");
        promptActivate = promptActivate && !shouldSkipActivatePrompt();
        return Promise.all([ getCurrentModule(), authStatusDetermined ]).then(function(_a) {
            var targetModule = _a[0];
            var activated = Promise.resolve();
            if (!user.isAuthenticated) {
                activated = promptToSignIn(container);
            } else if (user.isAuthenticated && promptActivate) {
                activated = promptToActivate(container);
            }
            return activated.then(function() {
                return renderSpinner(container, "Loading Azure Sandbox...");
            }).then(function() {
                return targetModule;
            });
        }).then(function(targetModule) {
            return sandboxApi.post(targetModule.uid, createReturnUrl()).then(function(result) {
                if (result.hasError === true) {
                    return renderKnownError(container, result.error).then(function() {
                        return promptAzureSandbox(container, false);
                    });
                }
                var sandbox = result.sandbox;
                if (sandbox.resourceGroupId) {
                    var _a = /^\/subscriptions\/([a-f0-9\-]{36})\/resourceGroups\/([\w\-]+)$/i.exec(sandbox.resourceGroupId), subscriptionId = _a[1], resourceGroupName = _a[2];
                ...
                if (!sandbox.resourceGroupId) {
                    return inviteToTenant(container, sandbox.invitationUrl);
                }
                if (sandbox.moduleId !== targetModule.uid) {
                    return getModule(msDocs.data.userLocale, sandbox.moduleId).then(function(currentModule) {
                        return promptToReleaseAzureSandbox(container, currentModule, targetModule);
                    }).then(function() {
                        return promptAzureSandbox(container, false);
                    });
                }
                container.setAttribute(stateAttr, state$1.finished);
                container.innerHTML = "<p>resourceGroupName: " + sandbox.resourceGroupName + "</p>\n\t\t\t\t\t<p>subscriptionId: " + sandbox.subscriptionId + "</p>\n\t\t\t\t\t<p>moduleExpiresAt: " + sandbox.moduleExpiresAt + "</p>";
                azureSandbox.value = sandbox;
                eventBus.publish(new AzureSandboxChangedEvent(sandbox));
                return sandbox;
            });
        }).catch(function() {
            return promptToRetry(container).then(function() {
                return promptAzureSandbox(container, false);
            });
        });
    }
    function shouldSkipActivatePrompt() {
        ...
        if (args[azureSandboxActivateParameter] === "true") {
            delete args[azureSandboxActivateParameter];
            updateQueryString(args, "replaceState");
            return true;
        }
        return false;
    ...
    function inviteToTenant(container, invitationUrl) {
        ...
        container.innerHTML = '<a href="' + invitationUrl + '">Accept invitation</a> to Azure Sandbox.';
        return new Promise(function() {});
    }
    function promptToReleaseAzureSandbox(container, currentModule, targetModule) {
        ...
        container.innerHTML = '\n\t\t<p>You have an Azure Sandbox for the <a href="' + currentModule.url + '">' + currentModule.title + '</a> module.\n\t\tWould you like to discard this sandbox and acquire a new Azure Sandbox for the "' + targetModule.title + '" module?</p>\n\t\t<button type="button">Yes</button>';
        return new Promise(function(resolve, reject) {
            container.querySelector("button").onclick = function() {
                renderSpinner(container, "Discarding Azure Sandbox...");
                sandboxApi.delete(currentModule.uid).then(resolve, reject);
            };
        });
    }
    function promptToRetry(container) {
        container.innerHTML = '\n\t\t<p>Something went wrong while creating your Azure Sandbox.</p>\n\t\t<button type="button">Retry</button>';
        return new Promise(function(resolve) {
            return container.querySelector("button").onclick = function() {
                return resolve();
            };
    ...
    function renderKnownError(container, error) {
        ...
        switch (error.errorCode) {
          case "MissingEmail":
            container.innerHTML = '\n\t\t\t\t<p>Unable to provision Azure Sandbox.\n\t\t\t\tYour account does not have an email address associated with it.\n\t\t\t\tPlease <a href="https://account.live.com" target="_blank">associate an email with your account</a>.</p>\n\t\t\t\t<button type="button">Retry</button>';
            return new Promise(function(resolve) {
                return container.querySelector("button").onclick = function() {
                    return resolve();
                };
          ...
          default:
            container.innerHTML = "Failed to load Azure Sandbox.";
            return new Promise(function() {});
        }
    }
    var sandboxApi = {
        base: "https://ppe.docs.microsoft.com/api/resources",
        post: function(moduleUid, returnUrl) {
            ...
            return fetch(sandboxApi.base + "/" + moduleUid, init).then(function(response) {
        ...
        delete: function(moduleUid) {
            ...
            return fetchWithTimeout(sandboxApi.base + "/" + moduleUid, init).then(function(response) {
                if (response.ok) {
                    return;
                }
                return Promise.reject();
    ...
    function enableRouter(args) {
        ...
        window$1.addEventListener("click", function(event) {
            ...
            event.preventDefault();
            render(anchor.href, location.href).then(function(result) {
                history.pushState({
                    url: result.url
        ...
        window$1.addEventListener("popstate", function(event) {
            return render(location.href, event.state.url);
        });
    }
    function render(url, referrerUrl) {
    ...
    function getClickInfo(event) {
        ...
        if (!anchor || event.altKey || event.ctrlKey || event.metaKey || event.shiftKey || getOrigin(anchor) !== location.origin || anchor.hasAttribute("download")) {
            return {
                shouldHandle: false,
                anchor: anchor
            };
    ...
    function moduleUnitPage() {
        ...
        var progressBadge = document$1.querySelector(".xp-tag.page-xp");
        ...
        if (!msDocs.data.context.chromeless && getMeta("azure_sandbox") === "true") {
            var alertContainer = showSandboxAlert();
            azureSandboxReady = promptAzureSandbox(alertContainer).then(function() {});
        }
        ...
        currentModule.then(function(module) {
            ...
            handleXpTag(progressBadge, currentUnit, bus);
            renderModuleNavigation(unitIndex, module, completionType, interactiveType ? interactiveType.name : null).then(function(navItems) {
                ...
                bus.subscribe(ProgressUpdatedEvent, function(event) {
                    updateNavigationItems(navigationItems, event.standardProgress);
                    updateXpTag(currentUnit.uid, progressBadge, event.standardProgress);
                });
                bus.subscribe(ProgressUpdatedWithPutEvent, function(event) {
                    updateNavigationItemViaPut(navigationItems, event.uidToUpdate);
                    updateXpTagWithPut(currentUnit.uid, progressBadge);
                });
                ...
                switch (completionType) {
                  ...
                  case "arm-task":
                    if (!msDocs.data.context.chromeless && getMeta("azure_sandbox") === "true") {
                        setupTaskValidation(unitId, bus);
                    }
                    break;
                }
            });
    ...
    function setupTaskValidation(unitId, bus) {
        authStatusDetermined.then(function() {
            ...
            validateButton.onclick = function() {
                ...
                if (azureSandbox.value) {
                    ...
                    combinedProgressService.validateTask(unitId, taskValidations);
                } else {
    ...
    function renderModuleNavigation(unitIndex, module, unitCompletionType, interactiveType) {
        return contentLoaded.then(function() {
            ...
            buildNextCTA(module.units, unitIndex, unitTotal, unitCompletionType, interactiveType);
    ...
    function buildNextCTA(unit, unitIndex, unitTotal, completionType, interactiveType) {
        var nextSection = document$1.getElementById("next-section");
        var nextMconstext = document$1.getElementById("next-module");
        var continueButton = document$1.getElementById("continue");
        var quizContinueButton = document$1.getElementById("quiz-modal-continue");
        var nextUnitUrl = "/" + msDocs.data.userLocale + unit[unitIndex + 1].url;
        continueButton.href = nextUnitUrl;
        if (completionType === "quiz") {
            quizContinueButton.href = nextUnitUrl;
        }
        if (msDocs.data.context.chromeless && completionType === "view" && interactiveType === "lab-on-demand") {
            continueButton.target = "_top";
        }
        nextMconstext.innerText = loc.next + ": " + unit[unitIndex + 1].title;
        if (unitIndex !== unitTotal - 1 && completionType === "view") {
            nextSection.classList.remove("is-hidden");
        }
    }
    function updateNavigationItems(navigationItems, progress) {
        var lookup = createProgressLookup(progress);
        navigationItems.forEach(function(item) {
            item.dataset.navUid in lookup ? item.classList.add("is-complete") : item.classList.remove("is-complete");
        });
        return navigationItems;
    }
    function updateNavigationItemViaPut(navigationItems, uidToUpdate) {
        var _a;
        var lookup = (_a = {}, _a[uidToUpdate] = true, _a);
        navigationItems.forEach(function(item) {
            if (item.dataset.navUid in lookup) {
                item.classList.add("is-complete");
            }
        });
        return navigationItems;
    }
    function showSandboxAlert() {
        var _a = msDocs.data, userDir = _a.userDir, userLocale = _a.userLocale;
        var metadataControl = document$1.querySelector(".page-metadata");
        var sandboxAlertHtml = '<div class="alert" dir="' + userDir + '" lang="' + userLocale + '"></div>';
